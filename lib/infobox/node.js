define(["moment", "numeral", "tablesort", "tablesort.numeric"],
  ...
  function showClientStats(d) {
    if (!d.flags.online)
      return undefined

    return function (el) {
      var canvas = document.createElement("canvas")
      canvas.id = "clientdiagram"
      canvas.width = "400"
      canvas.height = "200"
      canvas.onclick = function() {
        window.location.href = "/clients.html" + window.location.hash
      }
  ...
  return function(config, el, router, d) {
    ...
    attributeEntry(attributes, "Client-Statistik", showClientStats(d))
